...
export const CLIPFORGE_TURN_TOOL_CONTEXT =
  "ClipForge can use get_transcript and render_clip for the focused video. For every render_clip call, always provide a segments array, including for a single range. Use one precise segment per non-contiguous requested moment; never use one broad range that includes unrelated gaps. Up to 8 ordered segments can be joined by cuts or the fixed chapter-card transition. The tool also supports title, captions as true/false or explicit caption segments, output width, output height, and fps. Do not offer unsupported controls such as custom subtitle styling, fonts, colors, crop mode, fades, wipes, music, B-roll, thumbnails, hooks, or arbitrary effects.";
...
2. Before rendering, identify every precise source range needed by the request, remove unrelated gaps, and place those ranges in the segments array in output order. Then explain the intended clip briefly.
